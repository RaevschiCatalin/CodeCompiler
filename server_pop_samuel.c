...
#include <ctype.h>
#include <sys/wait.h>
#include <signal.h> // modificare Samuel Pop - pentru signal handling
...
#define MAX_JOBS 100
#define MAX_WORKERS 3 // număr maxim de execuții simultane

// samuel pop - structura job din coada
typedef struct {
    int client_sock;
    char header[512];
} Job;
...
volatile sig_atomic_t running = 1; // modificare Samuel Pop - flag pentru shutdown

void handle_signal(int signo) {
    running = 0;
} // modificare Samuel Pop - handler pentru SIGINT/SIGTERM
...
// samuel pop - adaugare job in coada
...
// samuel pop - worker thread care ia joburi din coada
...
int main() {
    signal(SIGINT, handle_signal);  // modificare Samuel Pop
    signal(SIGTERM, handle_signal); // modificare Samuel Pop
    ...
    for (int i = 0; i < MAX_WORKERS; ++i) {
        pthread_create(&workers[i], NULL, worker_thread, NULL); // samuel pop - porneste fire worker
    }
    ...
    while (running) { // modificare Samuel Pop
        int client_sock = accept(server_sock, (struct sockaddr *)&client_addr, &addr_size);
        if (client_sock < 0) {
            if (!running) break; // modificare Samuel Pop
            perror("accept");
            continue;
        }
        enqueue_job(client_sock); // samuel pop - pune job in coada
    }
    ...
    for (int i = 0; i < MAX_WORKERS; ++i) {
        ...
        pthread_join(workers[i], NULL); // samuel pop - asteapta terminare fire
    }
